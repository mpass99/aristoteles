...
function PARSER.parse(packet, tlv_tree, cur_tlv_data_byte, tlv_data_tvb, extra_information)
    ...
    local latitude = buffer(cur_tlv_data_byte, 2):le_uint()
    tlv_tree:add(fields.ibiumtscellinfot_psc, buffer(cur_tlv_data_byte, 2), latitude, "Primary Synchronization Code: " .. latitude)
    cur_tlv_data_byte = cur_tlv_data_byte + 2
